...
const char	*strSettingsText = "?????????"; 
const char	*strBrightText	 = "?????-??";
const char  *strTimeText		 = "????????";
const char  *strDateText		 = "???????";
const char	*strExitText		 = "-=Exit=-";

const char	*strDays[] = {"","пїЅпїЅ","??","??","??","??","??","??"};
...
void Graphic_Process()
{
	...
	if(currentTime->SubSeconds < 0x7F)
		sprintf(strOutBuf, "%02d изм.", 12);
	else
		sprintf(strOutBuf, "%02d ок.", 21);	
	...
	return;
		...
		{
			sprintf(strOutBuf,"20 пїЅпїЅпїЅ/пїЅ",adcBattery.percent);
			HCMS_PutStr(strOutBuf);
		}
		
		if(currensSysState == SYS_SET_BRIGHT)
		{
			sprintf(strOutBuf,"???:%.2d%??",Flash_Settings()->brightness);
			HCMS_PutStr(strOutBuf);
		}
